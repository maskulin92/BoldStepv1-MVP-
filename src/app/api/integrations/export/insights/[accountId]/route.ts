...
/**
 * GET /api/integrations/export/insights/[accountId]?format=json|csv&startDate=&endDate=&campaignId=
...
export const GET = withErrorHandling(async (request: Request, context: Context) => {
  ...
  if (format === 'csv') {
    return csvResponse(rows, `boldstep-insights-${accountId}-${startDate}_${endDate}.csv`);
  }
  if (format !== 'json') {
